...
public class Alfil extends PiezaAjedrez {
	...
	/**
	 * Constructor por defecto que construye un objeto 'Alfil' en la posición (4,4) de color BLANCO.
	 */
	public Alfil() {
		super();
	...
	/**
	 * Constructor de objeto 'Alfil' en la posición (x,y) y color que se indiquen en los parámetros.
	 * @param pos_x
	 * @param pos_y
	 * @param color
	...
	public boolean esMovibleA(int x, int y) {
		...
		if (this.pos_x>0 && this.pos_y>0 && this.pos_x<=8 && this.pos_y<=8)
			// El alfil está dentro del tablero, por lo que se puede mover.
			if (x>0 && y>0 && x<=8 && y<=8)
